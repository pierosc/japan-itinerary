...
async function saveTripOnline({
  ...
  try {
    ...
    // ✅ IMPORTANTE:
    // - Si tu tabla tiene UNIQUE(trip_id), esto funciona.
    // - No hacemos .select().single() para no cargar más ni arriesgar timeouts.
...
export default function App() {
  ...
  const handleBackToTrips = () => setActiveTripId(null);
  ...
  const performSave = async ({ silent = false } = {}) => {
    ...
    // Mantener data en memoria para resumen en Landing
    setTrips((prev) =>
      prev.map((t) =>
        t.id === activeTrip.id
  ...
  // ✅ Autosave: SOLO si autoSaveEnabled = true
  useEffect(() => {
    if (!activeTrip) return;
    if (storageMode !== "online") return;
    if (!autoSaveEnabled) return; // ✅ CLAVE
    if (!isSignedIn || !user?.id) return;

    const minutes = Math.max(1, Number(autoSaveIntervalMin) || 3);
